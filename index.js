...
const companions = {
  alteru: { nombre: "Altéru", clase: "Capitán", habilidad: "Rugido del León", coste: 500, nivel: 3 },
  ...
  nieriel: { nombre: "Nieriel", clase: "Capitán", habilidad: "Instinto de Supervivencia", coste: 150 },
  faelon: { nombre: "Faelon", clase: "Guardián Rúnico", habilidad: "Sabiduría de Imladris", coste: 100 },
  montaraces: { nombre: "Montaraces de Arathir", clase: "Cazadores", habilidad: "Exploradores del Norte", coste: 1000, nivel: 5 }
};
...
function getOwnedCompanions(profile) {
  return profile?.activeCompanions?.length
...
function getAffinityBonus(profile, companionId) {
  ...
  const value = affinity[companionId] || 0;
...
function getCompanionBonus(profile) {
  ...
  return {
    ...
    socialBonus: list.includes("faelon") ? 0.05 : 0,
...
async function companionReaction(companionId, context, mode = "encounter") {
  ...
  if (companionId === "faelon" && (normalizeKey(titulo).includes("elf") || normalizeKey(context?.id || "").includes("elf"))) {
...
Máximo 12 palabras.
Puede ser una frase o un gesto entre asteriscos.
Coloca tu nombre al inicio, así:
${nombre}: ...
...
`.trim();
  ...
  try {
    ...
    return `${nombre}: ${compactLine(raw, 12)}`;
  } catch {
    return `${nombre}: *asiente en silencio*`;
  }
}

async function companionReactions(profile, context, mode = "encounter", maxLines = 3) {
...
client.once("clientReady", async () => {
  ...
  setInterval(async () => {
    ...
    await announceDawnReset(client).catch(() => {});
  }, EXPEDITION_WINDOW_MS);
...
client.on("messageCreate", async (message) => {
  ...
  if (triviaGames.has(message.author.id)) {
    ...
    const correctNormalize = normalizeText(game.question.respuestaCorrecta || game.question.respuesta || "");

    if (textNormalize === correctNormalize) {
      triviaGames.delete(message.author.id);
      const points = game.difficulty === "facil" ? 10 : game.difficulty === "normal" ? 20 : game.difficulty === "dificil" ? 40 : 80;
      await db.addCorrectAnswer(message.author.id, points);
      await db.addXP(message.author.id, points * 2);
      return message.reply(`🎉 ¡Correcto! +${points} puntos y +${points * 2} XP.`);
    } else if (!command.startsWith("!")) {
      triviaGames.delete(message.author.id);
      await db.addWrongAnswer(message.author.id);
      return message.reply(`❌ Incorrecto. La respuesta correcta era: **${game.question.respuestaCorrecta || game.question.respuesta}**.`);
    }
  }
...
🤝 COMPAÑEROS
!campamento, !contratar <nombre>, !grupo
...
!a <mensaje> (Hablar con Altéru) o directos (!al, !c, !d, !an, !n, !f)`
  ...
  if (command === "!campamento") {
    ...
    for (const id of orden) {
      ...
      texto += `${getCompanionIcon(id)} **${comp.nombre}**\nClase: ${comp.clase || "Desconocida"}\nHabilidad: ${comp.habilidad}\nCoste: ${comp.coste} pts\nRequisito: ${comp.nivel ? `Nivel ${comp.nivel}` : "Ninguno"}\nPersonalidad: ${summarizePersonality(id)}\n\n`;
    }
  ...
  if (command === "!tablon") {
    ...
    for (const id of orden) {
      const comp = companions[id];
      texto += `${getCompanionIcon(id)} **${comp.nombre}** — ${comp.clase}\nHabilidad: ${comp.habilidad}\nCoste: ${comp.coste} pts | Requisito: ${comp.nivel ? `Nivel ${comp.nivel}` : "Ninguno"}\nPersonalidad: ${summarizePersonality(id)}\n\n`;
    }
  ...
  if (command === "!contratar") {
    ...
    const hireScene = { id: `hire_${id}`, titulo: `Contratación de ${companion.nombre}`, tipo: "evento_especial", categoria: "social", descripcion: `El viajero contrata a ${companion.nombre}.` };
    const reaction = await companionReaction(id, hireScene, "contratacion");
  ...
  if (command === "!grupo") {
    const profile = await db.getProfile(message.author.id);
    const list = getOwnedCompanions(profile);
    if (!list.length) return message.reply("No tienes compañeros contratados.");
  ...
  if (command === "!interactuar") {
    ...
    if (chosen) {
      ...
      if (reaction) texto += `\n\n💬 ${reaction}`;
      await db.addAffinity(message.author.id, chosen, 2);
    }

    for (const id of activeIds) {
      if (id !== chosen) await db.addAffinity(message.author.id, id, 1);
    }
...
${finalReactions ? `\n${finalReactions}` : ""}`.trim()
      ...
      if (owned.includes("nieriel")) {
        const safe = lista.filter(e => (e.peligro ?? 0) <= (nivelJugador + 1));
      ...
      let comandos = encounter.tipo !== "evento_especial" ? "\n\nComandos:\n!desafiar\n!volver" : "\n\nComandos:\n!interactuar\n!volver";
      ...
      for (const cid of reactionIds) {
        const line = await companionReaction(cid, encounter, "encuentro");
    ...
    if (success) {
      ...
      for (const cid of reactionIds) {
        ...
        if (line) reactions.push(`💬 ${line}`);
        await db.addAffinity(message.author.id, cid, 1);
      }
      ...
      if (expedition.progress < (expedition.mission.encuentros?.length || 0)) {
        ...
        if (finalReactions) textoVictoria += `\n\n${finalReactions}`;
  ...
  if (command === "!trivia") {
    ...
    if (attempts >= TRIVIA_LIMIT) return message.reply(`⚠️ Límite de ${TRIVIA_LIMIT} trivias alcanzado para este ciclo.`);

    const difficulty = args[1]?.toLowerCase() || "normal";
    const questions = await loadQuestions();
    const filtered = questions.filter(q => (q.dificultad || q.difficulty || "normal").toLowerCase() === difficulty);

    if (!filtered.length) return message.reply(`No hay preguntas de dificultad: **${difficulty}**.`);
    ...
    if (opciones) opciones.forEach((op, index) => { promptText += `\n${index + 1}️⃣ ${op}`; });

...
function pickExplorationItemByRarity(pool, rarity) {
  const filtered = (pool || []).filter(item => Utils.normalizeKey(item?.rareza || "comun") === rarity);
...
function getExplorationClosingText(points, item, rarity) {
  ...
  const rarityKey = Utils.normalizeKey(rarity || "");
...
async function handleExploracionCommand(message) {
  ...
  if (state.attempts >= EXPLORATION_LIMIT) {
    return message.reply(`⚠️ Ya usaste tu exploración. Vuelve en ${Utils.formatRemainingTime(state.resetAt - Date.now())}.`);
  }
...
function getPlayerClassKey(profile = {}) { 
  return Utils.normalizeKey(profile?.class || profile?.clase || ""); 
} 
...
async function addItemToInventory(userId, item) {
  ...
  const existingIndex = inventory[category].findIndex(x => Utils.normalizeKey(x.id) === Utils.normalizeKey(item.id));
...
async function findCatalogItemByQuery(query) {
  ...
  return pool.find(item => {
    ...
    const nombre = Utils.normalizeKey(item.nombre);
...
function buildPersonajesCache(input) {
  ...
  if (input && typeof input === "object") {
    return Object.fromEntries(
      Object.entries(input).map(([k, v]) => [Utils.normalizeKey(k), v])
    );
  }
  return {};
}
...
function getAffinityBonus(profile, companionId) {
  ...
  const value = affinity[Utils.normalizeKey(companionId)] || affinity[companionId] || 0;
...
function getCompanionBasePower(companionId) {
  ...
  for (const item of Object.values(loadout)) {
    ...
    if (typeof item === "string") {
      total += (ITEM_TIER_VALUES[Utils.normalizeKey(item)] || 0);
...
function getAffinityCombatBonus(profile, owned = []) {
  ...
  for (const compId of owned) {
    const value = affinity[compId] || affinity[Utils.normalizeKey(compId)] || 0;
...
function getFinalScenarioConfig(mission = {}, expedition = {}) {
  ...
  allowedActions = [...new Set(allowedActions.map(a => Utils.normalizeKey(a)))];
...
async function resolveFinalScenarioAction(message, expedition) {
  ...
  const normalizedAction = FINAL_SCENE_COMMANDS[commandKey] || Utils.normalizeKey(actionStr);
...
async function startFinalScenario(message, expedition) {
  ...
  let text = intro;
  if (reactions.length) text += `\n\n${reactions.join("\n")}`;
  return Utils.replyLong(message, text);
}
...
async function renderCatalogEmbed(catalogName, items, title, profile = {}, cycleId = 0, customHeader = "") {
  ...
  for (const item of items) {
    ...
    descripcion += `  **Precio:** ${Utils.formatPrice(price)} | **Slots:** ${remaining}/${maxSlots}\n`;
...
client.once("ready", async () => {
  ...
  try {
    ...
    personajesCache = Array.isArray(parsed)
      ? Object.fromEntries(parsed.filter(p => p && (p.id || p.nombre)).map(p => [Utils.normalizeKey(p.id || p.nombre), p]))
...
function parseClassChoice(input) {
  ...
  for (const [key, label] of Object.entries(CLASS_KEY_TO_LABEL)) {
    if (Utils.normalizeKey(key) === q || Utils.normalizeKey(label) === q) return key;
  }
  return null;
}
...
async function handleExpedicionStart(message, args) {
  ...
  if (state.attempts >= EXPEDITION_LIMIT) {
    return message.reply(`⚠️ Agotaste tus expediciones. Vuelve en ${Utils.formatRemainingTime(state.resetAt - Date.now())}.`);
  }
...
async function handleExpedicionDesafiar(message) {
  ...
  if (expedition.currentEncounter === null) {
    ...
    if (!encuentroId) {
      if (expedition.finalScenario?.enabled && !expedition.finalScenarioShown) {
        ...
        if (enemyPresent) {
         ...
         ...(typeof Combat.mapStatsToMatrixKeys === 'function' ? mapStatsToMatrixKeys(escenario.bonus || {}) : (escenario.bonus || {}))
       };
    ...
    const destino = Utils.normalizeKey(expedition.mission.destino);
    let lista = encounters.filter(e => {
      const coincideEncuentro = Utils.normalizeKey(e.tipo) === Utils.normalizeKey(encuentroId) || Utils.normalizeKey(e.categoria) === Utils.normalizeKey(encuentroId);
      const coincideRegion = Array.isArray(e.region) && e.region.some(r => Utils.normalizeKey(r) === destino);
...
🔥 Racha Actual: ${profile.rachaActual || 0} | Mejor: ${profile.mejorRacha || 0}` 
    ...
    Object.keys(companions).forEach(id => {
      ...
      txt += `${Utils.getCompanionIcon(id)} **${companions[id].nombre}**: ${val}/100 (${getAffinityRank(val)})\n`;
    });
    ...
    if (item.efecto?.salud) {
      ...
      if (activeUtils.some(u => Utils.normalizeKey(u.id) === Utils.normalizeKey(item.id))) {
    ...
    const idx = inventory[category].findIndex(x => Utils.normalizeKey(x.id) === Utils.normalizeKey(itemTabaco.id));
    if (idx !== -1) {
...
!a <mensaje> (Hablar con Altéru) o directos (!al, !c, !d, !an, !n, !f)` 
    ...
    for (const id of orden) {
      ...
      texto += `${Utils.getCompanionIcon(id)} **${comp.nombre}** — ${comp.clase}\n`;
    ...
    const equippedIds = Object.values(safeEquipment)
      .filter(Boolean)
      .map(x => x?.id ? Utils.normalizeKey(x.id) : null)
      .filter(Boolean);


    if (item?.id && equippedIds.includes(Utils.normalizeKey(item.id))) {
    ...
    for (const id of [...new Set(lista)]) {
      ...
      texto += `${Utils.getCompanionIcon(id)} **${companions[id]?.nombre || id}** — Afinidad ${valor}%\n`;
    }
    return message.reply(texto);
  }
  ...
  // ========================================
    ...
    let difficulty;
    if (args[1]) {
      difficulty = Utils.normalizeDifficulty(args[1]);
    } else {

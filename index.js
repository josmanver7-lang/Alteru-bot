...
async function getCatalogStateItems(stateKey, fallbackItems) {
  const state = await db.getEventState(stateKey).catch(() => null);
  ...
  return {
    state,
    items: selection.length ? selection : fallbackItems
  };
}
